<template>
  <el-card class="card-view box-card">
    <div class="card-header">
      <h1><i class="el-icon-date"></i> Sezonlar ({{ seasonList?.length }})</h1>
      ...
      <el-table-column fixed="right" label="İşlem" width="295">
        <template v-slot="scope">
          <el-button
            ...
            >Sezon Üretimlerini Gör</el-button
          >
          <el-button
            type="primary"
            icon="el-icon-edit"
...
export default {
  ...
  computed: {
    ...
    filteredData() {
      if (!this.filter.search) return this.seasonList
      return this.seasonList?.filter((item) =>
  ...
  methods: {
    async getAllSeasons() {
      ...
      if (error) {
        ...
        this.$message.error('Sezonlar yüklenirken bir hata oluştu.')
      } else {
    ...
    open(row) {
      this.$confirm(
        `${row.name} isimli sezonu, silmek istediğinden emin misiniz?`,
        'Sezon Silme İşlemi',
        {
